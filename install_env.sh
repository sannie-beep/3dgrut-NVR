...
CONDA_ENV_PATH=$(micromamba env list | sed -E -n "s/^${CONDA_ENV}[[:space:]]+\*?[[:space:]]*(.*)$/\1/p")
if [ -z "${CONDA_ENV_PATH}" ]; then
  echo "Conda environment '${CONDA_ENV}' not found, creating it"
  micromamba create --name ${CONDA_ENV} -y python=3.11
else
...
if [ "$WITH_GCC11" = true ]; then
    ...
    micromamba env config vars set CC=$GCC_11_PATH CXX=$GXX_11_PATH
...
micromamba env config vars set TORCH_CUDA_ARCH_LIST=$TORCH_CUDA_ARCH_LIST
micromamba deactivate
micromamba activate $CONDA_ENV
...
if [ "$CUDA_VERSION" = "11.8.0" ]; then
    ...
    micromamba install -y cuda-toolkit cmake ninja -c nvidia/label/cuda-11.8.0
    micromamba install -y pytorch==2.1.2 torchvision==0.16.2 torchaudio==2.1.2 pytorch-cuda=11.8 "numpy<2.0" -c pytorch -c nvidia/label/cuda-11.8.0
...
# CUDA 12.8 supports compute capability 10.0 and 12.0
    ...
    micromamba install -y cuda-toolkit cmake ninja gcc_linux-64=$GCC_VERSION -c nvidia/label/cuda-12.8.1
...
micromamba install -c conda-forge mesa-libgl-devel-cos7-x86_64 -y 
